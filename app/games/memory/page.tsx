...
export default function ColorMemoryGame() {
  ...
  const tiles: ColorTile[] = [
    { id: 0, name: 'Red', color: 'bg-rose-600', activeColor: 'bg-rose-300 brightness-125', positionClass: 'top-0 left-0 rounded-tl-full' },
    { id: 1, name: 'Blue', color: 'bg-sky-600', activeColor: 'bg-sky-300 brightness-125', positionClass: 'top-0 right-0 rounded-tr-full' },
    { id: 2, name: 'Green', color: 'bg-emerald-600', activeColor: 'bg-emerald-300 brightness-125', positionClass: 'bottom-0 left-0 rounded-bl-full' },
    { id: 3, name: 'Amber', color: 'bg-amber-500', activeColor: 'bg-amber-200 brightness-125', positionClass: 'bottom-0 right-0 rounded-br-full' },
  ];
  ...
  useEffect(() => {
    if (gameState === 'won') {
      const colors = ['#f43f5e', '#38bdf8', '#34d399', '#fbbf24'];
      ...
      {confettiPieces.map((p, idx) => (
        <div
          ...
          style={{
            ...
            left: `${p.x}px`,
            top: `${p.y}px`,
            width: `${idx % 3 === 0 ? p.size * 1.8 : p.size}px`,
            height: `${idx % 3 === 0 ? p.size * 2.2 : p.size * 0.6}px`,
            backgroundColor: idx % 3 === 0 ? 'transparent' : p.color,
            backgroundImage: idx % 3 === 0 ? 'radial-gradient(circle at 50% 40%, #ff7675 0%, #d63031 100%)' : 'none',
            transform: `rotate(${p.rotation}deg)`,
            borderRadius: idx % 3 === 0 ? '50% 50% 50% 50% / 40% 40% 60% 60%' : '2px',
            pointerEvents: 'none',
            zIndex: 50,
            opacity: 0.9,
          }}
